...
func Play(ctx oto.Context, song string, cat string) int {
	...
	fileBytes = config.NATS.GetBucket("mp3", song)
	/* 	if err != nil {
	...
	if decodedMp3err != nil {
		config.NATS.Send("messages."+StationId, "MP3 Decoder Error "+song, "onair")
...
func main() {
	...
	for {
		...
		if connectionspoolerr != nil {
			config.NATS.Send("messages."+*stationId, "Connection Pool Acquire FATAL "+connectionspoolerr.Error(), "onair")
		...
		if errscheduleget != nil {
			config.NATS.Send("messages."+*stationId, "Prepare Schedule Get FATAL "+errscheduleget.Error(), "onair")
		...
		for schedulerows.Next() {
			...
			if spinstoplayerr != nil {
				config.NATS.Send("messages."+*stationId, "Schedule spinstoplayerr "+spinstoplayerr.Error(), "onair")
				log.Panicln("Error spinstoplayerr "+scheduleerr.Error(), " schedule", playingday, playinghour, categories)
			}
			if scheduleerr != nil {
				config.NATS.Send("messages."+*stationId, "Schedule Get "+scheduleerr.Error(), "onair")
			...
			for spinstoplay > 0 {
				...
				if errinventorygetschedule != nil {
					...
					config.NATS.Send("messages."+*stationId, "Prepare Inventory Get "+errinventorygetschedule.Error(), "onair")
				}
				invrows, invrowserr = invgetconn.Query(context.Background(), "inventorygetschedule", categories)
				if invrowserr != nil {
					config.NATS.Send("messages."+*stationId, "Prepare Inventory Read "+invrowserr.Error(), "onair")
				...
				for invrows.Next() {
					...
					if inverr != nil {
						...
						config.NATS.Send("messages."+*stationId, "Inventory Song Get "+inverr.Error(), "onair")
					}
					// play the item
					config.NATSONAIR.SendONAIR(*stationId, artist+" - "+album+" - "+song)
					...
					if errinventoryupd != nil {
						...
						config.NATS.Send("messages."+*stationId, "Prepare Inventory Update "+errinventorygetschedule.Error(), "onair")
					}
					...
					if invupderr != nil {
						...
						config.NATS.Send("messages."+*stationId, "Inventory Update "+invupderr.Error(), "onair")
					}
					invupdconn.Release()
					if strings.HasPrefix(category, "ADDS") {
						...
						if trafficaddconnerr != nil {
							...
							config.NATS.Send("messages."+*stationId, "Prepare trafficadd conn "+trafficaddconnerr.Error(), "onair")
						...
						if errtrafficadd != nil {
							...
							config.NATS.Send("messages."+*stationId, "Prepare trafficadd "+errtrafficadd.Error(), "onair")
						}
						...
						if trafficadderr != nil {
							...
							config.NATS.Send("messages."+*stationId, "Updating Inventory "+trafficadderr.Error(), "onair")
						}
					...
					if exerr != nil {
						...
						config.NATS.Send("messages."+*stationId, "Inventory Time Parse "+exerr.Error(), "onair")
					}
					...
					if time.Now().After(ex) {
						...
						if errinventorydelete != nil {
							...
							config.NATS.Send("messages."+*stationId, "Prepage Indentory Delete "+errinventorydelete.Error(), "onair")
						}

						_, invdelerr = invdelconn.Exec(context.Background(), "inventorydelete", rowid)
						if invdelerr != nil {
							log.Println("deleting inventory " + invdelerr.Error())
							config.NATS.Send("messages."+*stationId, "Indentory Delete "+invdelerr.Error(), "onair")
						}
						invdelconn.Release()
						...
						intro = rowid + "INTRO"
						outro = rowid + "OUTRO"
						errremove = config.NATS.DeleteBucket("mp3s", fileid)
						if errremove != nil {
							log.Println("deleting  failed: ", errremove.Error(), fileid)
							config.NATS.Send("messages."+*stationId, "MP3 Bucket Delete "+fileid+" "+errremove.Error(), "onair")
						}
						errremovei = config.NATS.DeleteBucket("mp3s", intro)
						if errremovei != nil {
							log.Println("deleting  failed: ", errremovei.Error(), intro)
							config.NATS.Send("messages."+*stationId, "MP3 Bucket Delete "+intro+" "+errremove.Error(), "onair")
						}
						errremoveo = config.NATS.DeleteBucket("mp3s", outro)
						if errremoveo != nil {
							log.Println("deleting  failed: ", errremoveo.Error(), outro)
							config.NATS.Send("messages."+*stationId, "MP3 Bucket Delete "+outro+" "+errremove.Error(), "onair")
						}
					...
					if invrowserr != nil {
						...
						config.NATS.Send("messages."+*stationId, "Inventory Read "+invrowserr.Error(), "onair")
					}
					//conninv.Release()
		...
		if schedulerowserr != nil {
			...
			config.NATS.Send("messages."+*stationId, "Prepare Schedule Get rows error "+schedulerowserr.Error(), "onair")
		}
		schedulerows.Close()
		adjustToTopOfHour()
		getNextHourPart()
